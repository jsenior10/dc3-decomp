...
struct curl_hash *Curl_global_host_cache_init(void) {
    ...
    if (!host_cache_initialized) {
        rc = Curl_hash_init(
            &hostname_cache, 7, Curl_hash_str, Curl_str_key_compare, freednsentry
        );
        if (!rc)
...
static int remove_entry_if_stale(struct SessionHandle *data, struct Curl_dns_entry *dns) {
    ...
    Curl_hash_clean_with_criterium(
        data->dns.hostcache, (void *)&user, hostcache_timestamp_remove
    );
...
int Curl_resolv(
    struct connectdata *conn, const char *hostname, int port, struct Curl_dns_entry **entry
) {
    char *entry_id = NULL;
